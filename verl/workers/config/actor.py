...
class ActorConfig(BaseConfig):
    ...
    Args:
        ...
        ppo_mini_batch_size_per_gpu (Optional[int]): keep backward batch size when use different numbers of GPU.
    ...
    # add
    ppo_mini_batch_size_per_gpu: int = None
    metric: str = "acc"
    clip_type: str = "origin"
    ...
    def __post_init__(self):
        ...
        if not self.use_dynamic_bsz:
            if self.ppo_micro_batch_size is not None and self.ppo_micro_batch_size_per_gpu is not None:
                ...
                assert not (self.ppo_micro_batch_size is None and self.ppo_micro_batch_size_per_gpu is None), (
                    "[actor] Please set at least one of 'actor.ppo_micro_batch_size' or " "'actor.ppo_micro_batch_size_per_gpu' if use_dynamic_bsz is not enabled."
                )
        ...
        #     "only-token-mean",
        # ]
        if self.loss_agg_mode not in LossCalculator._value2member_map_:
    ...
    def validate(self, n_gpus: int, train_batch_size: int, model_config: dict = None):
        ...
        if not self.use_dynamic_bsz:
            if train_batch_size < self.ppo_mini_batch_size:
                raise ValueError(f"train_batch_size ({train_batch_size}) must be >= " f"actor.ppo_mini_batch_size ({self.ppo_mini_batch_size})")
            ...
            if self.ppo_micro_batch_size is not None:
                if self.ppo_mini_batch_size % self.ppo_micro_batch_size != 0:
                    raise ValueError(f"ppo_mini_batch_size ({self.ppo_mini_batch_size}) must be divisible by " f"ppo_micro_batch_size ({self.ppo_micro_batch_size})")
                if self.ppo_micro_batch_size * sp_size < n_gpus:
                    raise ValueError(f"ppo_micro_batch_size ({self.ppo_micro_batch_size}) * " f"ulysses_sequence_parallel_size ({sp_size}) must be >= n_gpus ({n_gpus})")
...
class FSDPActorConfig(ActorConfig):
    ...
    loss_normal_type: str = "micro_batch"
    def __post_init__(self):
    ...
    def validate(self, n_gpus: int, train_batch_size: int, model_config: dict = None):
        ...
        if self.strategy in {"fsdp", "fsdp2"} and self.ulysses_sequence_parallel_size > 1:
            if model_config and not model_config.get("use_remove_padding", False):
                raise ValueError("When using sequence parallelism for actor/ref policy, you must enable `use_remove_padding`.")
