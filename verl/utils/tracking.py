...
class Tracking:
    ...
    def __init__(self, project_name, experiment_name, default_backend: str | list[str] = "console", config=None):
        ...
        if "tensorboard" in default_backend:
            ...
            log_dir = os.environ.get("TENSORBOARD_DIR", f"/checkpoint_save/runs/{project_name}/{experiment_name}")
            self.logger["tensorboard_back_step"] = _TensorboardAdapter(project_name, experiment_name, log_dir)
            self.logger["tensorboard_gen_step"] = _TensorboardAdapter(project_name, experiment_name, log_dir + "/gen")
            self.logger["tensorboard_epoch"] = _TensorboardAdapter(project_name, experiment_name, log_dir + "/epoch")
        ...
        if "console" in default_backend:
            ...
            # console_logger = LocalLogger(print_to_console=True)
            # self.logger["console_back_step"] = console_logger
            # console_logger = LocalLogger(print_to_console=True)
            # self.logger["console_gen_step"] = console_logger
            # console_logger = LocalLogger(print_to_console=True)
            # self.logger["console_epoch"] = console_logger

        if "clearml" in default_backend:
            self.logger["clearml"] = ClearMLLogger(project_name, experiment_name, config)

    def log_all(self, data, back_step=None, gen_step=None, epoch=None, backend=None):
        for default_backend, logger_instance in self.logger.items():
            if "back_step" in default_backend and back_step is not None:
                logger_instance.log(data=data, step=back_step)
            elif "gen_step" in default_backend and gen_step is not None:
                logger_instance.log(data=data, step=gen_step)
            elif "epoch" in default_backend and epoch is not None:
                logger_instance.log(data=data, step=epoch)
            elif backend is None or default_backend in backend:
                logger_instance.log(data=data, step=back_step)
    ...
    def log(self, data, step):
        ...
        for k, v in data.items():
            ...
            if isinstance(v, int | float | np.floating | np.integer):
                ...
                logger.warning(f'Trainer is attempting to log a value of "{v}" of type {type(v)} for key "{k}". This ' f"invocation of ClearML logger's function is incorrect so this attribute was dropped. ")
...
class _TensorboardAdapter:
    def __init__(self, project_name, experiment_name, log_dir=None):
        import os
        from torch.utils.tensorboard import SummaryWriter

        if not log_dir:
            log_dir = os.environ.get("TENSORBOARD_DIR", f"/runs/{project_name}/{experiment_name}")
